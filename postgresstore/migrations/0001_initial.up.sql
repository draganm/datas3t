...
CREATE TABLE IF NOT EXISTS datas3ts (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    name TEXT NOT NULL UNIQUE,
    s3_bucket_id BIGINT NOT NULL,
    upload_counter BIGINT NOT NULL DEFAULT 0,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS dataranges (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (datas3t_id) REFERENCES datas3ts(id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS aggregate_uploads (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    datas3t_id BIGINT NOT NULL,
    upload_id VARCHAR(1024) NOT NULL,
    data_object_key VARCHAR(1024) NOT NULL,
    index_object_key VARCHAR(1024) NOT NULL,
    first_datapoint_index BIGINT NOT NULL,
    last_datapoint_index BIGINT NOT NULL,
    total_data_size BIGINT NOT NULL,
    source_datarange_ids BIGINT[] NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (datas3t_id) REFERENCES datas3ts(id) ON DELETE CASCADE
);
...
CREATE INDEX IF NOT EXISTS idx_dataranges_datas3t_id ON dataranges(datas3t_id);
CREATE INDEX IF NOT EXISTS idx_dataranges_key_range ON dataranges(min_datapoint_key, max_datapoint_key);
CREATE INDEX IF NOT EXISTS idx_aggregate_uploads_datas3t_id ON aggregate_uploads(datas3t_id);
