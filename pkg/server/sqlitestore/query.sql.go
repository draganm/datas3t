...
SELECT 
    d.name,
    COUNT(DISTINCT dr.id) as datarange_count,
    COALESCE(SUM(dr.size_bytes), 0) as total_size_bytes
FROM 
    datasets d
LEFT JOIN 
    dataranges dr ON d.name = dr.dataset_name
GROUP BY 
    d.name
ORDER BY 
    d.name
...
func (q *Queries) GetAllDatasets(ctx context.Context) ([]GetAllDatasetsRow, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&i.Name, &i.DatarangeCount, &i.TotalSizeBytes); err != nil {
			return nil, err
		}
