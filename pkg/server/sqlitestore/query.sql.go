...
const getDatarangesForMissingRanges = `-- name: GetDatarangesForMissingRanges :many
SELECT 
    min_datapoint_key, 
    max_datapoint_key
FROM dataranges 
...
type GetDatarangesForMissingRangesRow struct {
	MinDatapointKey int64
	MaxDatapointKey int64
}

func (q *Queries) GetDatarangesForMissingRanges(ctx context.Context, datasetName string) ([]GetDatarangesForMissingRangesRow, error) {
	rows, err := q.db.QueryContext(ctx, getDatarangesForMissingRanges, datasetName)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetDatarangesForMissingRangesRow
	for rows.Next() {
		var i GetDatarangesForMissingRangesRow
...
func (q *Queries) GetFirstAndLastDatapoint(ctx context.Context, datasetName string) (GetFirstAndLastDatapointRow, error) {
	row := q.db.QueryRowContext(ctx, getFirstAndLastDatapoint, datasetName)
	var i GetFirstAndLastDatapointRow
	err := row.Scan(&i.FirstDatapointKey, &i.LastDatapointKey)
	return i, err
}
